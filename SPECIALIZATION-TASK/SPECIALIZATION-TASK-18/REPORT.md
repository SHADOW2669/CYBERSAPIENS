...
> **Why the name change?**
> In the 2023 update, the focus shifted from the mechanism (**Rate Limiting**) to the consequence (**Resource Exhaustion**).
...
| **Performance Degradation** | High load causes latency, leading to a poor user experience. |
...
* **Rate Limiting:** Limit requests per timeframe (e.g., 5 login attempts per minute).
* **File Size Limits:** Configure web servers (like Nginx) to enforce `client_max_body_size`.
* **Input Validation:** Enforce maximum lengths for all JSON payloads and parameters.
* **Resource Quotas:** Set hard limits on CPU/Memory per container or process.
* **Monitoring:** Implement real-time alerts for spikes in traffic or 4xx/5xx error rates.
