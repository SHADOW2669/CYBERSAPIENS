...
* **Credential Stuffing:** Using leaked credentials from other breaches to brute-force login endpoints.
* **Weak Auth-Token Generation:** Using predictable or insecure session tokens that can be guessed or hijacked.
* **Unrestricted Password Changes:** Allowing a password update without requiring the user's current password, enabling attackers to lock legitimate users out.

### 2.3 Business Impact

* **Illegal Access:** Attackers gain full entry to end-user accounts and administrative panels.
* **Data Leakage:** Personal user data and proprietary organizational information can be exfiltrated.
* **Reputational Damage:** Loss of customer trust and potential legal penalties under data protection laws.
...
* **Observation:** The API only required the target username in the URL. It did not require proof of the original identity.
...
* **Conclusion:** The account was successfully compromised due to the lack of identity verification during sensitive changes.
...
1. **Require Current Password:** Enforce a "Current Password" field for all sensitive updates (passwords, emails, or MFA settings).
2. **Multi-Factor Authentication (MFA):** Implement MFA to ensure that even a compromised password does not grant full access.
3. **Implement Rate Limiting:** Prevent brute-force and credential stuffing by limiting login attempts.
4. **Secure Session Management:** Invalidate all active session tokens immediately after a password change to boot out any potential hijackers.
